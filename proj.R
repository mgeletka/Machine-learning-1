...
for (i in seq(along=ks)) {
  mod.train <- knn(train.x,train.x,k=ks[i],cl=train.set$diabetes)
  confussionMatrixTrain = table(mod.train, train.set$diabetes)
  accuracy.train[i] = sum(mod.train == train.set$diabetes)/length(train.set$diabetes)
  ...
  mod.test <- knn(train.x, test.x,k= ks[i],cl= train.set$diabetes)
  confussionMatrixTest = table(mod.test, test.set$diabetes)
  accuracy.test[i] = sum(mod.test == test.set$diabetes)/length(test.set$diabetes)
...
# Decision Boundaries_UPDATED

col1<-c('blue', 'magenta')

library(grid) 

plot.knn <- function(kneighbours, indexA, indexB) {
  grid.A <- seq(min(imput[,indexA]), max(imput[,indexA]), (max(imput[,indexA]) - min(imput[,indexA])) / 200)
  grid.B <- seq(min(imput[,indexB]), max(imput[,indexB]), (max(imput[,indexB]) - min(imput[,indexB])) / 100)
  grid <- expand.grid(grid.A,grid.B)
  colnames(grid) <- colnames(train.x[, c(indexA, indexB)])
  predicted.classes <- knn(train.x[, c(indexA, indexB)], grid, classes, k=kneighbours)
  plot(imput[, indexA], imput[ ,indexB], pch=20, col=col1[as.numeric(imput$diabetes)], xlab=colnames(imput)[indexA],     ylab=colnames(imput)[indexB])
...
plot.cart <- function(indexA, indexB) {
  ...
  grid <- expand.grid(grid.A,grid.B)
  colnames(grid) <- colnames(train.x[, c(indexA, indexB)])
  model1<-rpart(diabetes ~., data = imput[,c(indexA,indexB,9)], method = "class")
  
  predicted.classes <- predict(model1, grid, type="class")
  plot(imput[, indexA], imput[ ,indexB], pch=20, col=col1[as.numeric(imput$diabetes)], xlab=colnames(imput)[indexA],     ylab=colnames(imput)[indexB])
  points(grid[, 1], grid[,2], pch='.', col=col1[as.numeric(predicted.classes)])  # draw grid
  legend("topleft", legend=levels(imput$diabetes),fill =col1)
  title(c("Classification with CART"))
  predicted.matrix <- matrix(as.numeric(predicted.classes), length(grid.A), length(grid.B))
  contour(grid.A, grid.B, predicted.matrix, levels=c(1.5), drawlabels=FALSE,add=TRUE)
}
...
glm2 <- glm(diabetes~pregnant + glucose + mass + pedigree,family=binomial(logit),data=data)
summary(glm2)
