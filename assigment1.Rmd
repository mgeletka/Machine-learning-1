...
aggr(data,delimiter=NULL, plot=TRUE)
...
#### Glucose and age
Plot of the boundaries between columns glucose and age
In this plot we observe some datapoints misclassifed, a sign of underfitting - the model oversimplifies the problem and can't efficiently capture the underlying pattern of the data.
...
#### Pressure and mass
Plot of the boundaries between columns pressure and mass
When plotting pressure vs mass we observe more outliers.
...
```
###Boundaries of logistic regression in PCA
In an effort to bring out strong patterns from this datasets, we applied PCA. 
We see emerge two groupings for each class an they cluster together, with a stronger influence from the second component (PCA2).
```{r}
...
plot.linreg.boundaries <- function(indexA, indexB) {
  ...
  title(c("Classification with linear regression"))
   abline((1.5-ml$coefficients[1])/ml$coefficients[3],-ml$coeficcients[2]/ml$coefficients[3])
}
```
...
##Conclusion
From the evaluation metrics used, Linear Regression performed better than the others, nevertheless all models seemed adequate.
 
| Model               | Accuracy | Precision | Recall |
|---------------------|----------|-----------|--------|
| KNN                 | 0.749    | 0.8       | 0.810  |
| Logistic Regression | 0.766    | 0.809     | 0.838  |
| CART                | 0.734    | 0.826     | 0.792  |
| Linear Regression   | 0.778    | 0.826     | 0.841  |
