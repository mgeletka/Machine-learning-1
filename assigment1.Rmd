...
## Conclusion
In conclusion, after testing and checking this method and the possible difference with the hyperparameter of each method we can something more about how this method is placed inside the expressiveness grid.
Obviously, the cart and KNN method are the ones in which it is better to understand the use of hyperparameters, and how these parameters can change the result of the model.
In the knn case, we have to pay attention to choosing the right k, because of a k too small our algorithm behaves as overfitting and it gives a non-smooth decision surface( like the two regression), but also choosing a k too big we may end up smoothing things out too much and dropping some important details in the distribution.
Also in the CART method is there is a very important hyperparameter,
the stopping criteria, in our case we choose the minbucket that describe the smallest number of observations that are allowed in a terminal node. If a split decision breaks up the data into a node with less than the minbucket, it won’t accept it.